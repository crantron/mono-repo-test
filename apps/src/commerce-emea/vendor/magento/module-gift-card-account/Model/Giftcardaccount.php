...
class Giftcardaccount extends \Magento\Framework\Model\AbstractExtensibleModel implements GiftCardAccountInterface
{
    ...
    public const STATE_REDEEMED = 2;

    public const STATE_EXPIRED = 3;

    public const REDEEMABLE = 1;

    public const NOT_REDEEMABLE = 0;
    ...
    public const BASE_GIFT_CARDS_AMOUNT = 'base_gift_cards_amount';

    public const GIFT_CARDS_AMOUNT_USED = 'gift_cards_amount_used';

    public const BASE_GIFT_CARDS_AMOUNT_USED = 'base_gift_cards_amount_used';
    ...
    public function beforeSave()
    {
        ...
        if (false === is_numeric($this->getBalance())) {
            throw new LocalizedException(
                __('Specified balance should be a numeric value.')
            );
        }
    ...
    public function redeem($customerId = null)
    {
        ...
        if (!$customerId) {
            throw new LocalizedException(__('Please enter a valid customer ID.'));
        }
        $this->redeemer->redeem($this->getCode(), (int) $customerId);
     ...
     * @return void
     * @deprecated 101.0.0
     * @see EmailManagement::sendEmail() instead
     */
    ...
    {
        $this->_logger->critical(new LocalizedException(__($realMessage)));
        if (!$fakeMessage) {
            $fakeMessage = 'Please correct the gift card code.';
        }
        throw new LocalizedException(__($fakeMessage));
    }
