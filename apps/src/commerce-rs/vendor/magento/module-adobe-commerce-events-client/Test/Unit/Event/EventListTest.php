...
class EventListTest extends TestCase
{
    ...
    public function testValidateEventData()
    {
        ...
        $this->expectExceptionMessage('Wrong configuration in "io_events" section of app/etc/config.php file for the '.
            'event "xml_event1_no_eventData". The configuration must be in array format with at least one '.
            'field configured.');

        $this->eventList->validateEventData('xml_event1_no_eventData', []);
    }
        ...
        $this->eventFactoryMock->expects(self::exactly(4))
            ...
            ->withConsecutive(
                ...
                [[
                    ...
                    'optional' => true,
                    'enabled' => false,
                    'fields' => [['name'=>'name', 'converter'=>null]],
                    'rules' => [],
                    'priority' => 1,
                    'destination' => Event::DESTINATION_DEFAULT,
                    'processors' => []
