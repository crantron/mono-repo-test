...
class Operation extends \Magento\Framework\Model\AbstractModel
{
    ...
    protected function _afterLoad()
    {
        ...
        if ($fileInfo !== null && trim($fileInfo)) {
    ...
    /**
     * Check if data has valid file path.
     *
     * @return void
     * @throws LocalizedException
     */
    private function validateFilePath(): void
    {
        ...
        if (isset($fileInfo['file_path']) && preg_match('/(?:^-|\s-)/', $fileInfo['file_path'])) {
            throw new LocalizedException(
                __('Invalid characters are not allowed as the file path.')
            );
        }
    }
